...
typedef struct {
    ...
    pthread_t               thread;
    int                     control[2];
    char                    excludeNmeaSentences[PROPERTY_VALUE_MAX];
} GpsState;

static GpsState  _gps_state[1];

// special constants stored in GpsSvInfo::size to track the state
// of satellites across multiple GSA/GSV sentences
#define PRN_SIZE_UNUSED 0
#define PRN_SIZE_SEEN_BUT_UNUSED 1
#define PRN_SIZE_USED 2

#define PRN_MAX 1023
static GpsSvInfo   prn_state[PRN_MAX + 1] = {{PRN_SIZE_UNUSED}};

// Circular buffer of PRN of most recently seen satellites.
// Binary 0 means unseen prn
static int    most_recent_prns[GPS_MAX_SVS];
// Circular index. 1 means we've seen up to
// GPS_MAX_SVS with most recent PRN stored at index most_recent_prns[1]
static int    prn_counter = 0;
//#define  GPS_DEBUG  1
...
static int
nmea_reader_update_prns( NmeaReader*  r, int inview, int num, int i, Token prn, Token elevation, Token azimuth, Token snr )
{
    int o;
    int prnid;
    i = (num - 1)*4 + i;
    if (i < inview) {
        const int sat_prn = str2int(prn.p,prn.end);
        if (sat_prn > PRN_MAX)
        {
            D("Prn too high: %d, ignoring ", sat_prn);
        }
        else
        {
            prn_state[sat_prn].prn = sat_prn;
            prn_state[sat_prn].elevation = str2int(elevation.p,elevation.end);;
            prn_state[sat_prn].azimuth = str2int(azimuth.p,azimuth.end);
            prn_state[sat_prn].snr = str2int(snr.p,snr.end);
            prn_state[sat_prn].size = PRN_SIZE_SEEN_BUT_UNUSED;
            prn_counter = (prn_counter + 1) % GPS_MAX_SVS;
            most_recent_prns[prn_counter] = sat_prn + 1;
        }
    }
    return 0;
}
...
nmea_reader_parse( NmeaReader*  r )
{
    ...
    */
    ...
    const size_t tokenLength = tok.end - tok.p;
    memcpy(tokenString, tok.p, tokenLength);
    tokenString[tokenLength] = 0;
    if (strstr(_gps_state->excludeNmeaSentences, tokenString))
    {
        D("Excluded by filter: %s", tokenString);
        return;
    }
    ...
    if ( !memcmp(tok.p, "GGA", 3) ) {
        ...
        if (!nmea_reader_update_accuracy(r, tok_accuracy))
        {
        ...
        */
        ...
        for ( i=0; i<12; i++ ) {
            ...
            if ( tok_id.end > tok_id.p ){
                const int prn = str2int(tok_id.p,tok_id.end);
                if (PRN_MAX < prn)
                {
                    D("Prn too high: %d, ignoring", prn);
                }
                else if (prn_state[prn].size)
                {
                    prn_state[prn].size = PRN_SIZE_USED;
        ...
        unsigned u;
        for (u = prn_counter;
             r->sv_status.num_svs < GPS_MAX_SVS && most_recent_prns[u];
             u = (u - 1) < (GPS_MAX_SVS - 1) ? (u - 1) : (GPS_MAX_SVS - 1))
        {
            int prn = most_recent_prns[u] - 1;
            // special size 1 indicates seen but not used for solution satellite
            if (PRN_SIZE_UNUSED != prn_state[prn].size)
            {
                r->sv_status.sv_list[r->sv_status.num_svs] = prn_state[prn];
                r->sv_status.sv_list[r->sv_status.num_svs].size = sizeof(r->sv_status.sv_list[i]);
                if (PRN_SIZE_SEEN_BUT_UNUSED != prn_state[prn].size)
                {
                    r->sv_status.used_in_fix_mask |= (1ul << r->sv_status.num_svs);
                }
                ++r->sv_status.num_svs;
            }
        }

//        // reset sizes back to 'unused' so that next GSA does not get confused
//        for ( i=0; i<r->sv_status.num_svs; i++ ) {
//            prn_status[r->sv_status.sv_list[i].prn].size = 1;
//        }
...
//            update_gps_svstatus(&r->sv_status);
        ...
        nmea_reader_update_prns( r, svs_inview, msg_number, 0, tok_sv1_prn_num, tok_sv1_elevation, tok_sv1_azimuth, tok_sv1_snr );
        nmea_reader_update_prns( r, svs_inview, msg_number, 1, tok_sv2_prn_num, tok_sv2_elevation, tok_sv2_azimuth, tok_sv2_snr );
        nmea_reader_update_prns( r, svs_inview, msg_number, 2, tok_sv3_prn_num, tok_sv3_elevation, tok_sv3_azimuth, tok_sv3_snr );
        nmea_reader_update_prns( r, svs_inview, msg_number, 3, tok_sv4_prn_num, tok_sv4_elevation, tok_sv4_azimuth, tok_sv4_snr );
    ...
    static const uint16_t hasSomething =
...
//            GPS_LOCATION_HAS_BEARING |
    ...
    if (hasSomething == (r->fix.flags & hasSomething)) {
...
gps_state_init( GpsState*  state, GpsCallbacks* callbacks )
{
    ...
    property_get("ro.kernel.android.gpsexclnmea",state->excludeNmeaSentences,"");
